...
- [x] Interactive TUI (`dtctl tui`) — Phase 1 of [TUI_DESIGN.md](TUI_DESIGN.md): k9s-style navigator with problems/services/hosts/logs/events views, scoped drill-downs (l/m/p/v), tabbed entity detail pages (key facts + metrics/logs/events/problems tabs), searchable record inspector with relevance highlights, canned metrics charts, command bar, timeframe picker, command echo

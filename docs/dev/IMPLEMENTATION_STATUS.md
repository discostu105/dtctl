...
- [x] Interactive TUI (`dtctl tui`, ships as the separate `dynatui` binary) — Phases 1–3 of [the dynatui design doc](../../dynatui/docs/design/tui.md): k9s-style navigator with a home triage view, problems/services/hosts/logs/events, the Kubernetes catalog (pods, workloads, namespaces, nodes, clusters with ready/restart/phase columns), traces with a span waterfall (log↔trace jumps), AWS inventory + generic entity browser, frontends (RUM), Postgres databases, GenAI entities, security vulnerabilities; Smartscape relations panel (x), scope pinning (.), batched sparkline enrichment, column sorting, digit hotkeys, DQL escape hatch with reveal-query (ctrl+q), browser deep links via intents (o), yank/copy (y/c), tabbed entity detail pages, searchable record inspector, command bar with argument filters, timeframe picker, command echo

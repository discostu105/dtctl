# dynatui Split & Plugin System — Design Proposal

**Status:** Phase 1 executed 2026-07-10 — in-repo split: the TUI lives in its
own Go module and binary (`dynatui/`, module `github.com/dynatrace-oss/dynatui`),
`dtctl tui` forwards to the `dynatui` binary on PATH, and the root module is
...
`pkg/{config,auth,client,safety}` as re-export shims; dynatui consumes
...
Split the TUI out of dtctl into a separate project (working name **dynatui**) at the
...
**dtctl owns config and context management entirely; dynatui is a pure
...
dependency: a container with a baked-in config file runs dynatui alone.

Additionally, adopt a **lean kubectl-style exec plugin convention** for dtctl.
dynatui ships as its first plugin (`dtctl-tui`), which preserves the `dtctl tui`
...
- **TUI size**: ~12k LOC excluding tests (~17k including) across `pkg/tui/`
  and `cmd/tui*.go`. (Since Phase 1 the code lives in `dynatui/internal/tui/`
  plus the `dynatui` main package.)
- **Coupling surface is already thin** (by design — see the dynatui design doc
...
- **The sdk has never been consumed as a real dependency**: the root `go.mod`
  ...
  pseudo-version, and no `sdk/v*` tags exist. dynatui would be its **first true
  external consumer**.
...
- **`DTCTL_CONTEXT` exists only in tests** — there is no production env-var
  ...
  dtctl (`LoadConfig`) and dynatui (flag > env > workspace match > file), always
...
2. **Enforce the seam** — ✅ done 2026-07-10 (Phase 1): the TUI is its own Go
   module + binary inside the repo (`dynatui/`), which yields the lean-dtctl and
...
- **Safety-level semantics.** If a `readonly` context must mean the same thing
  in both tools (the dynatui design doc says it does), safety is part of the shared
...
- **Terminal renderers** (sparkline/braille/chart) — the "no display logic in
  ...
  module with zero Dynatrace coupling, or let dynatui fork them and diverge
  toward lipgloss-native implementations (the dynatui design doc already flags the
  two-styling-systems tension; divergence may be healthy).
- Resource display-field metadata; the TUI's ViewSpec catalog is dynatui-domain.

**Pragmatic bridge** (the Phase-1 mechanism): `pkg/` is not `internal/`, so
dynatui can legally import `github.com/dynatrace-oss/dtctl/pkg/config` etc. from
the root module with zero refactoring — this is how the in-repo `dynatui/` module
consumes dtctl today (via `replace` directives), and how a separate dynatui repo
...
## Decision 3 — Config & context: dtctl owns management entirely; dynatui is a pure consumer

(Revised 2026-07-10: sharpened from "shared contract with dynatui
...
| Option | Verdict | Why |
|---|---|---|
| dynatui gets its own config/credentials | ❌ | Double onboarding, drifting contexts, two credential stores; kills "drop into the TUI wherever dtctl points" |
| dtctl as installed prerequisite (shell out) | ❌ | A TUI makes dozens of concurrent cancellable calls — process-spawn latency, output-parsing fragility, version skew; handing tokens across process boundaries is worse than in-process keyring reads. Note Go has no runtime code sharing: an installed dtctl binary can serve dynatui **only** via shell-out, so "prerequisite" buys nothing architecturally |
| **dtctl owns all config/context management; dynatui consumes it by importing dtctl's Go packages** | ✅ | k9s/kubeconfig model; existing users get dynatui working instantly with zero migration; dynatui ships no onboarding, no `ctx` CRUD, no config-write path — the whole two-writer config contract disappears |

Consequences (Phase-1 state in parentheses):

- dynatui reads `~/.config/dtctl/config` and keyring service `"dtctl"` through
...
- **All context management lives in dtctl**: create/edit/delete, login flows,
  safety-level assignment, every write to the config file. dynatui's first-run
  message with no contexts points at `dtctl ctx create`. "Install dtctl" is a
  soft prerequisite in the docs — dynatui never checks for or invokes the binary.
- dynatui may **switch** between existing contexts session-locally
  (implemented: `dynatui --context` overrides in memory, never persists) — the
  same line k9s draws: switch in-app, create/edit elsewhere.
- dynatui gets its **own namespace for UI-only state**: `~/.local/state/dynatui/`
  (implemented: view history lives in `~/.local/state/dynatui/history.json`,
  migrated on first run from `~/.local/state/dtctl/tui-history.json`). Auth
  and contexts never live there.
- The shared config file is **read-only for dynatui**. The **token store is the
  one exception**: OAuth refresh tokens rotate on use, so a long-running dynatui
  must persist refreshed token sets or it strands dtctl's stored copy —
  dynatui is unavoidably a token-store *writer*, through the same locked refresh
...
1. **OAuth forced-refresh bypasses the refresh lock.** (Corrected 2026-07-10:
   ...
   that method. A long-running dynatui racing a concurrent dtctl refresh on the
   same context can strand one side's credentials. This also settles Decision
   3's scope: dynatui is read-only for the config file but necessarily a
...
2. **Current-context is shared mutable state.** Today `dtctl query --context X`
   *persists* the context switch to disk. If dynatui inherits write-through
   semantics, an open TUI where the user hits `:ctx staging` silently repoints
   every script and agent using dtctl on that machine. dynatui context switches
...
5. **Client identity.** dynatui needs its own `User-Agent` (`pkg/version` is
...
the dynatui split were not happening, plugins would be premature (third-party
...
is already required for dynatui, and the dispatcher converts the dynatui migration
...
1. **The dynatui shipping vehicle**: dynatui ships from its own repo as `dtctl-tui`
   (packaging alias `dynatui`); `dtctl tui` keeps working via dispatch.
...
- **No ecosystem is coming soon.** Success metric: "dynatui ships cleanly and
...
- Prerequisite worth doing regardless: make `DTCTL_CONTEXT` a **real
  ...
  contract. ✅ Done 2026-07-12 (dtctl and dynatui; session-local by contract).
...
- **Embedded scripting (Lua/Starlark/WASM)**: a different problem. Note:
  k9s-style user-customizable views (YAML ViewSpecs) is a **dynatui** feature on
  dynatui's roadmap — do not conflate TUI extensibility with CLI plugins.
...
4. **Enforce the seam** — ✅ done 2026-07-10 (Phase 1, pulled ahead of steps
   1–3 via the pragmatic bridge): own Go module + binary in-repo (`dynatui/`),
   `dtctl tui` forwards, root module TUI-free. Narrowing dynatui's imports from
   `pkg/*` to the promoted sdk ✅ done 2026-07-12 (dynatui consumes
...
5. **Split** when TUI PRs stop touching `pkg/`/`sdk/` in the same change:
   `git filter-repo` → dynatui repo; `dtctl tui` becomes the forwarder; dynatui is
...
- **Maintainer math**: two repos = two CI pipelines, release flows
  ...
  fine and intended.
- **Version skew**: users will run dtctl X + dynatui Y. The config schema version
  plus a documented compat statement ("dynatui N supports config schema ≤ M") is
  the answer; test it with the shared golden fixtures.
- **Ecosystem may never materialize**: acceptable; the plugin mechanism is
  cheap and pays for itself via dynatui + scope defense alone.

## Open Questions

1. **Upstream relationship**: is the TUI (and the sdk session-layer
   refactoring) intended for dynatrace-oss upstream, or is dynatui an independent
   project consuming the published sdk? If upstream stalls, the root-module
   import bridge (Decision 2) is the interim path.
2. **Naming**: check "dynatui" for collisions (registries, GitHub, brew) before
   branding.
3. **Renderer strategy**: extract a standalone terminal-viz module vs fork
   into dynatui and let implementations diverge.
4. **Does dynatui ever write the shared config?** Resolved 2026-07-10: the
   config file — never (dtctl owns all management; no dynatui onboarding is
...
- `dynatui/docs/design/tui.md` — the TUI design this proposal extracts
- `dynatui/docs/dev/learnings.md` — field notes; moved to dynatui with the code
  2026-07-10 (dynatui docs restructured into design/adr/dev 2026-07-12)

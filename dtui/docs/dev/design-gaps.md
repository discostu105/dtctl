...
> **Scope rule (decided 2026-07-14):** dtui is **strictly read-only** —
...
1. **User-assignable hotkeys (`:hotkeys`)** — tui.md's movement table calls
   ...
   map in dtui's state dir, `:hotkey <digit> <view>` to assign. Until then
...
- **History path** is `~/.local/state/dtui/history.json` (dtui's own
...
`:nav <name>` resolution + `dtui nav <type|id|name>` · `:ctx <name>`

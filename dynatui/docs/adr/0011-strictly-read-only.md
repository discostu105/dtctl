# ADR-0011: dynatui is strictly read-only
...
**dynatui never mutates anything. Mutations are not deferred — they are not
...
- The safety level stays in the header — it is still the identity of the
  context the user is pointed at — but it gates nothing inside dynatui.
- The `c` command-echo key is the mutation path: dynatui teaches the CLI

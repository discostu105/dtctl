...
- **`:nav <name>` resolves names** (and `dynatui nav <type|id|name>` from the
...
- **Query history**: the escape hatch remembers submitted DQL (MRU, capped
  at 50, persisted to `~/.local/state/dynatui/queries.json`); `ctrl+p`/`ctrl+n`
...
> **Decided 2026-07-14 — dynatui is strictly read-only; mutations are not

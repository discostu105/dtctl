...
dynatui is a keyboard-driven terminal navigator for Dynatrace. It makes a
...
The audience is not a job title. dynatui serves the person **already in a
...
| Verb | Tool | Meaning |
|---|---|---|
| **Navigate & explore** | dynatui | see what exists and what is happening: entities, topology, signals, and read-only assets — settings included, because *reading* config is exploring the tenant |
...
dynatui's job ends when you know **which entity** and **which timeframe**
...
4. **Zero configuration** ([ADR-0010](../adr/0010-curated-defaults-runtime-discovery.md)).
   dynatui works on an arbitrary tenant out of the box: curated defaults plus
...
6. **Strictly read-only** ([ADR-0011](../adr/0011-strictly-read-only.md)).
   dynatui never mutates anything; the command echo is the handover.
...
- **Not a monitoring daemon** — dynatui is a session tool measured in minutes:

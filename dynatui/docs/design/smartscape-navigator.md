...
- **Management zones don't exist in Grail Smartscape** — out of scope; would
  ...
  deliberately does not add (DQL-only keeps the TUI seam thin for the dynatui
  split — see the dtctl repo’s DYNATUI_SPLIT_DESIGN.md).
...
a pure-TUI feature that does not widen the dynatui split seam.
...
history (trail survives restore). Name entry (`:nav <name>`, `dynatui nav
...
- [dtctl DYNATUI_SPLIT_DESIGN.md](../../../docs/dev/DYNATUI_SPLIT_DESIGN.md) — seam discipline this feature respects

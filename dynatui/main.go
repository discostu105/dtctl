// Command dynatui is the interactive terminal UI for Dynatrace — a k9s-style
// navigator over observability primitives (see docs/design/tui.md).
//
// dynatui is a pure consumer of dtctl's configuration: contexts and credentials
// are created and managed with dtctl (`dtctl ctx create`), and dynatui reads the
// same config file and keyring through the shared session layer
// (sdk/session; docs/dev/DYNATUI_SPLIT_DESIGN.md, Decisions 2+3). It never
...
import (
	...
	"github.com/dynatrace-oss/dynatui/internal/tui"
	"github.com/dynatrace-oss/dynatui/internal/tui/catalog"
)

// version is stamped by the build (see `make build-dynatui`).
var version = "dev"
...
var rootCmd = &cobra.Command{
	...
	Long: `dynatui is an interactive terminal UI — a k9s-style navigator over
...
dynatui reads dtctl's contexts and credentials; create and manage them with
dtctl (dtctl ctx create). dynatui is read-only and interactive-only.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if !noAgent {
			if info := aidetect.Detect(); info.Detected {
				return fmt.Errorf("dynatui is interactive-only and does not run in AI agent sessions (pass --no-agent if this session was misdetected)")
			}
		}
		if !term.IsTerminal(int(os.Stdout.Fd())) {
			return fmt.Errorf("dynatui requires a terminal (stdout is not a TTY)")
		}

		// A committed .dynatrace.yaml supplies workspace defaults; a broken
		// one must never brick dynatui — errors degrade to a startup warning.
		...
		if len(args) == 2 {
			// Only the navigator takes an argument (`dynatui nav <type|id|name>`,
			// smartscape-navigator.md) — everywhere else the second word is a
			// mistake worth naming.
			if !isNavView(view) {
				return fmt.Errorf("only nav takes an argument (dynatui nav <type|id|name>); %q does not", view)
			}
			viewArg = args[1]
		}
		...
		// either way — dynatui never writes the shared config.
		...
		// dynatui ships its own client identity (DYNATUI_SPLIT_DESIGN.md Landmine 5):
		// tenant-side request logs must distinguish dynatui from dtctl.
		c, err := session.NewClientFromConfig(cfg, session.WithUserAgentProduct("dynatui", version))
		if err != nil {
			return err
		}
...
// flag or DTCTL_CONTEXT env var) is applied in memory only — dynatui never
...
// honors, so `DTCTL_CONTEXT=x dtctl ...` and `DTCTL_CONTEXT=x dynatui` agree).
...
// only — dynatui never writes the config, the same session-local contract as
...
// historyPath returns dynatui's navigation-history file. UI state lives in
// dynatui's own namespace — never in dtctl's — so the first run migrates the
// history written by the pre-split `dtctl tui`.
func historyPath() string {
	path := filepath.Join(xdg.StateHome, "dynatui", "history.json")
...
func queryHistoryPath() string {
	return filepath.Join(xdg.StateHome, "dynatui", "queries.json")
}

// loadWorkspace discovers the project's .dynatrace.yaml. Errors degrade to a
// warning and a nil workspace: a broken committed file must never brick dynatui.
